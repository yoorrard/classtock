...
const TeacherRegisterModal: React.FC<TeacherRegisterModalProps> = ({ onClose, onRegisterSuccess, addToast, onSwitchToLogin }) => {
    ...
    const [step, setStep] = useState<'enterDetails' | 'verifyCode'>('enterDetails');
    const [sentCode, setSentCode] = useState('');
    const [verificationCode, setVerificationCode] = useState('');
    const [error, setError] = useState('');
    
    const handleGoogleAuth = () => {
        // For demo, just register and log in
        onRegisterSuccess('google_user@example.com', 'google_password');
    };
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                <p style={{marginTop:0, marginBottom: '1rem'}}>서비스를 이용하시려면 회원가입이 필요합니다.</p>
                ...
                {step === 'verifyCode' ? (
                     <form onSubmit={handleVerifyAndRegister}>
                        <p style={{textAlign: 'center', marginBottom: '1.5rem', lineHeight: 1.6}}>
                            회원가입을 완료하려면, <strong>{email}</strong>(으)로 발송된<br/>6자리 인증번호를 입력해주세요.
                        </p>
                        ...
                        <button type="submit" className="button" style={{ width: '100%', marginBottom: '1rem' }}>인증하고 회원가입</button>
                         <button type="button" className="button-link" style={{marginTop: 0}} onClick={() => { setStep('enterDetails'); setError(''); setVerificationCode(''); }}>이메일 주소 변경</button>
                    </form>
                ) : (
                    <form onSubmit={handleSendCode}>
                        <div className="input-group"><input type="email" className="input-field" placeholder="이메일 주소" aria-label="이메일 주소" value={email} onChange={(e) => setEmail(e.target.value)} required /></div>
                        
                        <div className="input-group" style={{ position: 'relative' }}>
                            <input type={showPassword ? 'text' : 'password'} className="input-field" placeholder="비밀번호" aria-label="비밀번호" value={password} onChange={(e) => { setPassword(e.target.value); setError(''); }} required />
                        ...
                        <div className="input-group">
                             <div style={{ position: 'relative' }}>
                                <input type={showConfirmPassword ? 'text' : 'password'} className="input-field" placeholder="비밀번호 확인" aria-label="비밀번호 확인" value={confirmPassword} onChange={(e) => { setConfirmPassword(e.target.value); setError(''); }} required />
                                 <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} style={{ position: 'absolute', right: '1rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#666', padding: 0, lineHeight: 1 }} aria-label={showConfirmPassword ? "비밀번호 숨기기" : "비밀번호 보기"}>
                                    {showConfirmPassword ? EyeClosedIcon : EyeOpenIcon}
                                </button>
                            </div>
                            {passwordMismatch && <p className="error-message" style={{ marginTop: '0.25rem', marginBottom: 0 }}>비밀번호가 일치하지 않습니다.</p>}
                        </div>
                        ...
                        <button type="submit" className="button" style={{ width: '100%', marginBottom: '1rem' }} disabled={!email || !password || passwordMismatch}>회원가입</button>
                    </form>
                )}

                <button type="button" className="button-link" style={{marginTop: 0}} onClick={onSwitchToLogin}>
                    이미 계정이 있으신가요? 로그인
                </button>
