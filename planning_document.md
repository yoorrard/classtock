...
> **※ 현재 구현 상태**: 기능 구현 및 테스트를 위해 백엔드 및 데이터베이스는 React의 로컬 상태(State)로, 주가 데이터는 사전에 정의된 가상(Mock) 데이터로 운영되고 있습니다. Firebase 및 KIS API 연동은 배포 버전의 목표 사양입니다. 또한, 브라우저의 로컬 스토리지(Local Storage)가 주가 정보의 일일 업데이트 여부 및 팝업 공지 확인 여부를 기록하는 데 사용됩니다.
...
#### 3.2.1. 회원가입 및 로그인
- 이메일/비밀번호 또는 소셜 로그인(Google)을 통해 Firebase Authentication 기반의 회원가입 및 로그인을 수행합니다.
- 비밀번호 분실 시 이메일을 통한 재설정 기능을 제공합니다.
- **관리자 히든 로그인**: 교사 로그인 창에서 관리자 계정을 입력하면 관리자 페이지로 이동합니다.
...
#### 3.4.1. 관리자 로그인
- 랜딩 페이지에 별도의 관리자 로그인 버튼을 노출하지 않습니다 (히든 로그인).
- **교사 로그인** 모달창에서 지정된 관리자 아이디(`admin@classstock.com`)와 비밀번호(`admin`)를 입력하면 관리자 대시보드로 진입합니다.
...
- **데이터 관리**:
  - (목표) 모든 사용자 정보, 학급 데이터, 거래 내역 등은 Firebase Firestore 데이터베이스에 안전하게 저장 및 관리됩니다.
...
### 5.3. 관리자 플로우
1. 랜딩 페이지의 '교사 로그인' 버튼 클릭.
2. 이메일(`admin@classstock.com`)과 비밀번호(`admin`) 입력.
3. 관리자 대시보드로 이동.
4. '공지사항 관리', 'Q&A 관리', '팝업 공지 관리' 탭에서 서비스 콘텐츠 관리.
5. 사용자 문의 확인 및 답변 작성 또는 게시글 관리.